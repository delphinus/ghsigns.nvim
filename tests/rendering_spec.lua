...
describe("PR content rendering", function()
  ...
  describe("PR #4 full rendering", function()
    ...
    before_each(function()
      -- Read PR #4 body from file if available, otherwise use inline
      local body
      local f = io.open("/tmp/pr4_body.txt", "r")
      if f then
        body = f:read "*a"
        f:close()
      else
        -- Fallback: skip test if body file not available
        pending "PR #4 body file not available at /tmp/pr4_body.txt"
        return
      end
